    ...
    >
      ...
      <div className="w-full max-w-[1080px] xl:max-w-[1200px] 2xl:max-w-[1320px] mx-auto px-6 md:px-12 relative z-10">
        ...
        <div className="flex items-end justify-between gap-6 md:gap-12 mb-14 [@media(max-width:1000px)]:mb-10">
          <div>
            <p className="text-xs font-black tracking-[0.25em] text-[var(--color-navy)] mb-5">
              CONTACT
            </p>
        ...
        <div className="grid grid-cols-2 gap-10 [@media(max-width:1000px)]:grid-cols-1 [@media(max-width:1000px)]:gap-8">
          {[
            { ...cta.phone, label: "電話でお問い合わせ" },
            { ...cta.email, label: "メールでお問い合わせ" },
          ].map((c) => (
            <a key={c.value} href={c.href} className="block group">
              <p className="text-[0.85rem] font-bold text-[var(--color-text-muted)] mb-2 tracking-wider">
                {c.label}
              </p>
              <p className="text-[1.6rem] md:text-[1.9rem] font-black text-[var(--color-navy)] group-hover:text-[var(--color-primary)] transition-colors break-all [@media(max-width:1000px)]:text-[1.35rem]">
                {c.value}
              </p>
            </a>
          ))}
        ...
        <div className="mt-14 flex flex-wrap items-center gap-4 [@media(max-width:1000px)]:mt-10">
          ...
          <p className="text-[var(--color-text-muted)] font-bold text-[0.9rem]">{cta.url}</p>
        </div>
